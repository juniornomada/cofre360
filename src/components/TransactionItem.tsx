...
export function TransactionItem({ 
  ...
  return (
    <div
      onClick={onEdit}
      className={cn(
        "interactive-card flex items-center gap-3 rounded-2xl p-3.5 cursor-pointer bg-card/40 hover:bg-card/60 border border-border/20 transition-all group/item relative overflow-hidden active:scale-[0.98] sm:pr-3 pr-[48px] shadow-sm hover:shadow-md"
      )}
      style={style}
    >
      {onDelete && (
          ...
          >
            {onDelete && (
              <button
                ...
                className="p-1.5 rounded-full hover:bg-destructive/10 text-muted-foreground hover:text-destructive transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-destructive focus-visible:ring-offset-1"
                aria-label="Excluir"
              >
