...
export function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 border-t border-border bg-card/80 backdrop-blur-xl">
      <div className="mx-auto flex max-w-md items-center justify-between gap-1 px-2 py-2 pb-[max(0.5rem,env(safe-area-inset-bottom))] overflow-x-auto no-scrollbar">
        {navItems.map((item) => {
              ...
              <TooltipTrigger asChild>
                <Link
                  ...
                  className={cn(
                    ...
                    isActive
                      ? "text-primary bg-primary/20 shadow-sm ring-1 ring-primary/30"
                ...
                >
                  ...
                  {isActive && (
                    <span className="h-1.5 w-1.5 mt-0.5 rounded-full bg-primary animate-scale-in" />
                  )}
                </Link>
              </TooltipTrigger>
