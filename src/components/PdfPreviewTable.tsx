...
export function PdfPreviewTable({ rows, onChange, itemLabel = "transações", rawPdfText, documentKind = "card_invoice" }: Props) {
  ...
  const hasDivergence = (r: PdfPreviewRow) => {
    ...
    return Math.abs(numOrig - r.amount) > divThreshold;
  };

  const isLowConfidence = (r: PdfPreviewRow) => {
    return (r.confidence_score ?? 100) < confThreshold;
  };
        ...
        <div className="flex items-center justify-between gap-2 px-1">
          ...
          <Popover>
            <PopoverTrigger asChild>
              <Button variant="ghost" size="sm" className="h-7 w-7 p-0 rounded-lg" title="Configurar limiares">
                <Settings2 className="h-3.5 w-3.5" />
              </Button>
            </PopoverTrigger>
            <PopoverContent className="w-64 p-3 bg-popover border border-border shadow-xl rounded-xl" align="end">
              <div className="space-y-4">
                <div className="space-y-2">
                  ...
                  <p className="text-[9px] text-muted-foreground leading-tight">
                    Transações com confiança da IA abaixo deste valor serão marcadas para revisão.
                  </p>
                </div>
                
                <div className="space-y-2 border-t pt-3 border-border/50">
                  <div className="flex justify-between text-[10px] font-medium uppercase tracking-wider text-muted-foreground">
                    <span>Divergência Máxima</span>
                    <span>R$ {divThreshold.toFixed(2)}</span>
                  </div>
                  <Input 
                    type="number" 
                    step="0.01" 
                    min="0"
                    value={divThreshold} 
                    onChange={(e) => setDivThreshold(Number(e.target.value))}
                    className="h-7 text-xs px-2 rounded-md"
                  />
                  <p className="text-[9px] text-muted-foreground leading-tight">
                    Marca para revisão se a diferença entre o PDF e a extração for maior que este valor.
                  </p>
                </div>
              </div>
            </PopoverContent>
