    ...
    <div className="animate-page-enter flex flex-col gap-4 px-4 pt-6 pb-24">
      ...
      <Dialog open={showAddDialog} onOpenChange={(open) => {
        ...
        <DialogContent className="max-w-[90vw] rounded-2xl bg-background">
          ...
          <div className="flex flex-col gap-3">
            {/* Type selector — always visible at top */}
            <div>
              <label className="text-xs text-muted-foreground mb-1 block">Tipo</label>
              <div className="flex gap-2">
                <button
                  ...
                  className={`flex-1 rounded-xl py-2 text-xs font-medium transition-colors ${!isTransfer && newTx.type === "expense" ? "bg-destructive text-destructive-foreground" : "bg-card text-muted-foreground"}`}
                >
                  Despesa
                </button>
                <button
                  ...
                  className={`flex-1 rounded-xl py-2 text-xs font-medium transition-colors ${!isTransfer && newTx.type === "income" ? "bg-primary text-primary-foreground" : "bg-card text-muted-foreground"}`}
                >
                  Receita
                </button>
                <button
                  ...
                  className={`flex-1 rounded-xl py-2 text-xs font-medium transition-colors flex items-center justify-center gap-1 ${isTransfer ? "bg-blue-500 text-white" : "bg-card text-muted-foreground"}`}
                >
            ...
            {isTransfer ? (
              <>
                ...
                <div className="rounded-xl bg-card/50 p-3 space-y-3">
                  <div>
                    <label className="text-xs text-muted-foreground mb-1 block">De (conta de origem)</label>
                    <select
                  ...
                  <div className="flex justify-center">
                    <span className="flex h-7 w-7 items-center justify-center rounded-full bg-blue-500/15 text-blue-500">
                      <ArrowRight className="h-4 w-4" />
                    </span>
                  </div>
                  <div>
                    <label className="text-xs text-muted-foreground mb-1 block">Para (conta de destino)</label>
                    <select
                      value={transferToId}
                    ...
                    >
                      <option value="">Selecione...</option>
                      {bankAccounts.filter(a => a.id !== transferFromId).map(a => <option key={a.id} value={a.id}>{a.name}</option>)}
                    </select>
                  </div>
                  {transferFromId && transferToId && transferFromId === transferToId && (
                    <p className="text-[11px] text-destructive">A conta de origem e destino devem ser diferentes.</p>
                  )}
                </div>
                <div>
                  <label className="text-xs text-muted-foreground mb-1 block">Data</label>
                  <Popover>
                    <PopoverTrigger asChild>
                      <Button variant="outline" className={cn("w-full justify-start text-left font-normal rounded-xl bg-card border-none", !newTx.date && "text-muted-foreground")}>
                        <CalendarIcon className="mr-2 h-4 w-4" />
                        {newTx.date || "Selecionar data"}
                      </Button>
                    </PopoverTrigger>
                    <PopoverContent className="w-auto p-0" align="start">
                ...
                <div>
                  <label className="text-xs text-muted-foreground mb-1 block">Valor (R$)</label>
                  <input type="number" step="0.01" value={newTx.amount || ""} onChange={e => setNewTx({ ...newTx, amount: parseFloat(e.target.value) || 0 })} placeholder="0.00" className="w-full rounded-xl bg-card px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground outline-none" />
                </div>
              </>
            ) : (
              <>
                <div className="relative">
                  ...
                  <input
                    ...
                    onChange={e => {
                      ...
                      setShowAddSuggestions(e.target.value.length >= 2);
                    }}
                    onFocus={() => setShowAddSuggestions(newTx.name.length >= 2)}
                  ...
                  />
                  {showAddSuggestions && getAutocompleteSuggestions(newTx.name).length > 0 && (
                    <div className="absolute z-50 left-0 right-0 top-full mt-1 rounded-xl bg-popover border border-border shadow-lg max-h-48 overflow-y-auto">
                      {getAutocompleteSuggestions(newTx.name).map((s, i) => (
                        <button
                          key={i}
                          type="button"
                          onMouseDown={(e) => e.preventDefault()}
                          onClick={() => {
                            setNewTx({ ...newTx, name: s.name, icon: s.icon, category: s.category });
                ...
                <div>
                  <label className="text-xs text-muted-foreground mb-1 block">Cartão de Crédito</label>
                  <select value={newTx.card || "Nenhum"} onChange={e => setNewTx({ ...newTx, card: e.target.value === "Nenhum" ? null : e.target.value })} className="w-full rounded-xl bg-card px-3 py-2 text-sm text-foreground outline-none">
                    {cardOptions.map(c => <option key={c.name} value={c.name}>{c.name}</option>)}
                  </select>
                </div>
                {newTx.card && newTx.card !== "Nenhum" && (
                  <div className="space-y-2 rounded-xl bg-card/50 p-3">
                    <div className="flex items-center justify-between">
                      <label className="text-xs text-muted-foreground">Parcelar</label>
                      <button
                        type="button"
                        onClick={() => setInstallmentEnabled(!installmentEnabled)}
                        className={`relative h-5 w-9 rounded-full transition-colors ${installmentEnabled ? "bg-primary" : "bg-muted"}`}
                      >
                        <span className={`absolute top-0.5 left-0.5 h-4 w-4 rounded-full bg-white transition-transform ${installmentEnabled ? "translate-x-4" : ""}`} />
                      </button>
                    </div>
                    {installmentEnabled && (
                      <>
                        <div className="flex gap-2">
                          <button type="button" onClick={() => setInstallmentMode("divide")} className={`flex-1 rounded-xl py-1.5 text-xs font-medium transition-colors ${installmentMode === "divide" ? "bg-primary text-primary-foreground" : "bg-card text-muted-foreground"}`}>
                            Dividir total
                          </button>
                          <button type="button" onClick={() => setInstallmentMode("fixed")} className={`flex-1 rounded-xl py-1.5 text-xs font-medium transition-colors ${installmentMode === "fixed" ? "bg-primary text-primary-foreground" : "bg-card text-muted-foreground"}`}>
                            Valor fixo/parcela
                          </button>
                        </div>
                        <div className="flex gap-2 items-end">
                          <div className="flex-1">
                            ...
                            <input type="number" min={2} max={48} value={installmentCount} onChange={e => setInstallmentCount(Math.max(2, parseInt(e.target.value) || 2))} className="w-full rounded-xl bg-card px-3 py-2 text-sm text-foreground outline-none" />
                          </div>
                          {installmentMode === "fixed" && (
                            <div className="flex-1">
                              <label className="text-xs text-muted-foreground mb-1 block">Valor/parcela</label>
                              <input type="number" step="0.01" value={installmentFixedValue || ""} onChange={e => setInstallmentFixedValue(parseFloat(e.target.value) || 0)} placeholder="0.00" className="w-full rounded-xl bg-card px-3 py-2 text-sm text-foreground outline-none" />
                            </div>
                          )}
                        </div>
                          <div className="space-y-1.5 mt-1">
                ...
                <div>
                  <label className="text-xs text-muted-foreground mb-1 block">Conta Bancária</label>
                  <select value={newTx.bank_account_id || ""} onChange={e => setNewTx({ ...newTx, bank_account_id: e.target.value || null })} className="w-full rounded-xl bg-card px-3 py-2 text-sm text-foreground outline-none">
