...
import { ArrowLeft, Plus, CreditCard, Trash2, X, Check, Loader2, Wallet, Landmark, ChevronLeft, ChevronRight, Receipt, FileUp, GripVertical, Layers, Pencil, MoreVertical, Eye, EyeOff, Copy, AlertCircle, CheckCircle2, Info, Search, SlidersHorizontal, CalendarIcon, Trash, ChevronDown, ChevronUp, RotateCcw, Inbox } from "lucide-react";
...
function CardsPage() {
  ...
  const openInvoiceDialog = async (card: CardData) => {
    ...
    try {
      ...
      const [, txResult, refundsResult] = await Promise.all([
        fetchAll(),
        supabase
          .from("transactions")
  ...
  const carriedPendingRefunds = getCarriedPendingRefundsForPeriod(activePeriod);
        ...
        >
          ...
          {loadingTx ? (
            ...
            <div className="flex flex-col flex-1 min-h-0">
              ...
              >
                {activePeriod && activePeriod.transactions.length === 0 && activeOpeningInvoiceAmount <= 0 && carriedPendingRefunds.length === 0 ? (
                  <InvoiceEmptyState
                  ...
                  <div className="flex flex-col gap-0.5" data-testid="invoice-transactions-list">
                    {carriedPendingRefunds.length > 0 && (
