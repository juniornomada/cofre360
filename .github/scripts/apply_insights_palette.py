...
old = '''const assistantListTone = (value: string) => {
...
new = '''const assistantListTone = (value: string) => {
  const text = value.toLocaleLowerCase("pt-BR");
...
if old_strong not in text:
    raise SystemExit("strong renderer anchor not found")
text = text.replace(old_strong, new_strong, 1)
